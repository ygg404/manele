...
<script>
  ...
  export default {
    ...
    created () {
      this.dataForm.userAccount = this.$cookie.get('yggAccount')
      this.dataForm.password = this.$cookie.get('yggPwd')
    },
    methods: {
      // 提交表单
      dataFormSubmit () {
          ...
          if (valid) {
            ...
            this.$http({
              ...
              if (data && data.code === 0) {
                ...
                this.$cookie.set('yggAccount' , this.dataForm.userAccount)
                this.$cookie.set('yggPwd' , this.dataForm.password)
